...
public class MapServer {
    ...
    public static List<String> getLocationsByPrefix(String prefix) {
        ...
        TieSet trie = new TieSet();
        Iterator<String> i = graph.way.keySet().iterator();
        while(i.hasNext()){
            String name = i.next();
            if(name.equals("")){
                continue;
            }
            trie.put(GraphDB.cleanString(name));
        }
        trie.find(prefix);
        String[] parts = trie.suffix.toString().split("-");
        for(int x = 0; x < parts.length; x++){
            returnList.add(prefix+parts[x]);
        }
        return returnList;
    }
